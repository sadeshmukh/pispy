...
export const POST: APIRoute = async ({ request, redirect }) => {
	...
	if (action === "create") {
		...
		const { rows } = await db.execute(
			"SELECT MAX(field_order) as max FROM clue_fields",
		);
		...
		for (const id of ids) {
			if (deleteIds.has(id)) {
				await db.execute({
					sql: "DELETE FROM user_clues WHERE field_id = ?",
					args: [id],
				});
				await db.execute({
					sql: "DELETE FROM clue_fields WHERE id = ?",
					args: [id],
				});
				continue;
			}
			...
			if (!prompt) continue;
			const fieldOrder =
				parseInt(String(formData.get(`field_order_${id}`))) || 0;
			const difficulty = normalizeDifficulty(formData.get(`difficulty_${id}`));
			const questionType = normalizeQuestionType(
				formData.get(`question_type_${id}`),
			);
			const placeholder = String(
				formData.get(`placeholder_${id}`) ?? "",
			).trim();
			await db.execute({
				sql: "UPDATE clue_fields SET prompt = ?, field_order = ?, difficulty = ?, question_type = ?, placeholder = ? WHERE id = ?",
				args: [prompt, fieldOrder, difficulty, questionType, placeholder, id],
...
function normalizeDifficulty(value: FormDataEntryValue | null) {
	return ["hard", "medium", "easy"].includes(String(value))
		? String(value)
		: "medium";
}
